...
def generate_grid():
    return [[random.randint(1, 99) for _ in range(GRID_SIZE)] for _ in range(GRID_SIZE)]
...
def hill_climbing(grid):
    ...
    for step in range(ITERATIONS):
        ...
        history.append({"step": step + 1, "energy": current_energy})
...
def genetic_algorithm(grid):
    ...
    for generation in range(ITERATIONS):
        ...
        history.append({"step": generation + 1, "energy": best_energy})
...
def optimize():
    ...
    results = [hc, sa, ga]
    best_result = max(results, key=lambda item: item["best_energy"])
